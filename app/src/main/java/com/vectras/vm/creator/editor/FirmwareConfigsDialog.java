...
public class FirmwareConfigsDialog extends BottomSheetDialogFragment {
    ...
    private void initialize() {
        ...
        if (!MainSettingsManager.getArch(requireContext()).equals(MainSettingsManager.X86_64_ARCH) && !MainSettingsManager.getArch(requireContext()).equals(MainSettingsManager.I386_ARCH)) {
            ...
            binding.cbvOpencore.setVisibility(View.GONE);
            ...
            binding.cbvUseuefi.setOnCheckedChangeListener((v, isChecked) -> {
                configs.isUseUefi = isChecked;
                if (!isChecked && binding.cbvOpencore.isChecked()) {
                    binding.cbvOpencore.setChecked(false);
                    configs.isOpenCore = false;
                }
            });

            if (MainSettingsManager.getArch(requireContext()).equals(MainSettingsManager.X86_64_ARCH)) {
                binding.cbvOpencore.setOnCheckedChangeListener((v, isChecked) -> {
                    configs.isOpenCore = isChecked;
                    if (isChecked && !binding.cbvUseuefi.isChecked()) {
                        binding.cbvUseuefi.setChecked(true);
                        configs.isUseUefi = true;
                    }
                });
            } else {
                binding.cbvOpencore.setVisibility(View.GONE);
        ...
        binding.cbvUseDefaultBios.setOnCheckedChangeListener((v, isChecked) -> {
            ...
            binding.cbvOpencore.setEnabled(isChecked);
        });
    ...
    private void load() {
        ...
        if (MainSettingsManager.getArch(requireContext()).equals(MainSettingsManager.X86_64_ARCH)) {
            ...
            binding.cbvOpencore.setChecked(configs.isOpenCore);
        } else {
            binding.cbvUseuefi.setVisibility(View.GONE);
            binding.cbvOpencore.setVisibility(View.GONE);
        }
        ...
        binding.cbvOpencore.setEnabled(configs.isUseDefaultBios);
    ...
    private void save() {
        ...
        configs.isOpenCore = binding.cbvOpencore.isChecked();
